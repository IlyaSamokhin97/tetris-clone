...
spawnPiece d = d { state = Spawned pos, field = spawn (piece d) pos (field d) }
  where pos = (0, 5)

spawned :: Maybe Turn -> Maybe Move -> Bool -> (Int, Int) -> Data -> Data
spawned mt mov hasTicked pos d = 
  let
    (newPiece, newField, newPos) =
      let
        (piece', field', pos') = fromMaybe (piece d, field d, pos) (mt      >>= tryTurnPiece pos  (piece d) (field d))
        (field'', pos'')       = fromMaybe (defaultField, pos')    (moveDir >>= tryMovePiece pos' piece'    field'   )
          where
            (defaultField, moveDir) = if hasTicked
    ...
    newField'   = newField // [(Pos (y, x), Disappearing) | y <- frozenRowYs, x <- frozenRowXs]
      where
        frozenRowYs = [y | (y, cells) <- zip [0..] rows, all (==Frozen) cells]
...
blinkDisappearingRows doShow blinks dt d =
  ...
  then
    let
      (timer, doFlash) = tick dt $ clearRowFlashTimer d
    in d
      { clearRowFlashTimer = timer
      , state = if doFlash
...
draw :: Data -> Canvas -> IO ()
draw dat canvas = do
  fieldPos <- getFieldPos canvas
  Draw.clear canvas
  Draw.field (field dat)                    fieldPos            cellPx (drawDisappearing dat) canvas
  Draw.field (blueprint $ next $ piece dat) (piecePos fieldPos) cellPx False                 canvas
  where
    cellPx = 30

    getFieldPos c = do
      (_, (ch, cw)) <- getBounds c
      return (ch `div` 2 - (fh * cellPx) `div` 2, cw `div` 2 - (fw * cellPx) `div` 2)

    piecePos (y, x) =
      (y, x + fw * cellPx + cellPx)

    (_, Pos (fh, fw)) = bounds $ field dat
    
    drawDisappearing d = case state d of
...
tryFall field = do
  floatingY <- find isFloating . reverse $ [0 .. height - 2]
  let ys = [0..floatingY]
  Just $ field
  ...
  where
    ...
    isFloating y = any (==Falling) [field ! Pos (y, x)     | x <- xs]
                && all (==Empty)   [field ! Pos (y + 1, x) | x <- xs]
